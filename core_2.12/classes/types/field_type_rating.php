...
class field_type_rating extends field_type_default
{
	...
	public function addThanks($user_id, $record_graph_top = false){	
		...
		$rec = $this->model->modules[ $module_sid ]->getRecordById('rec', $user_id);
		$rec = $this->model->modules[ $module_sid ]->explodeRecord($rec, 'rec');
		
		//Добавляем голос в запись
		foreach($rec as $field_sid => &$field){
			if( $this->model->modules[ $module_sid ]->structure['rec']['fields'][ $field_sid ]['type'] == 'rating' ){
				@$field['thanks']++;
				$rating = $this->toValue($field_sid, $rec);
				$this->model->execSql('update `'.$this->model->modules[ $module_sid ]->getCurrentTable('rec').'` set `'.$field_sid.'`="'.$rating.'" where `id`="'.$rec['id'].'" limit 1', 'update');
			}
		}
		
		//Возвращаем URL обновлённой записи
		return $this->model->modules[ $module_sid ]->insertRecordUrlType( $rec['url'] );
	}
