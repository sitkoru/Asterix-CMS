...
* @package  OLE
*/
class PHPExcel_Writer_Shared_OLE
{
    /**
    ...
    * @access public
    */
    function PHPExcel_Writer_Shared_OLE()
    {
        $this->_list = array();
    }
    ...
    */
    function read($file)
    {
        ...
        $i1stBdL = ($big_block_size - 0x4C) / PHPExcel_Writer_Shared_OLE::OLE_LONG_INT_SIZE;
    ...
    */
    function _readPpsWks($pps_wk_start, $big_block_size)
    {
        ...
        while (1)
        {
            ...
            $pps_wk = fread($this->_file_handle, PHPExcel_Writer_Shared_OLE::OLE_PPS_SIZE);
            if (strlen($pps_wk) != PHPExcel_Writer_Shared_OLE::OLE_PPS_SIZE) {
                break; // Excel likes to add a trailing byte sometimes 
                //throw new Exception("PPS at $pointer seems too short: ".strlen($pps_wk));
            }
            ...
            if (($type[''] != PHPExcel_Writer_Shared_OLE::OLE_PPS_TYPE_ROOT) and
                ($type[''] != PHPExcel_Writer_Shared_OLE::OLE_PPS_TYPE_DIR) and
                ($type[''] != PHPExcel_Writer_Shared_OLE::OLE_PPS_TYPE_FILE))
            {
                throw new Exception("PPS at $pointer has unknown type: {$type['']}");
            }
            ...
            $this->_list[] = new PHPExcel_Writer_Shared_OLE_PPS(null, '', $type[''], $prev[''], $next[''],
                                         $dir[''], PHPExcel_Writer_Shared_OLE::OLE2LocalDate($time_1st),
                                         PHPExcel_Writer_Shared_OLE::OLE2LocalDate($time_2nd),
            ...
            $pointer += PHPExcel_Writer_Shared_OLE::OLE_PPS_SIZE;
        }
    }
    ...
    */
    function isFile($index)
    {
        if (isset($this->_list[$index])) {
            return ($this->_list[$index]->Type == PHPExcel_Writer_Shared_OLE::OLE_PPS_TYPE_FILE);
        }
        return false;
    }
    ...
    */
    ...
    {
        if (isset($this->_list[$index])) {
            return ($this->_list[$index]->Type == PHPExcel_Writer_Shared_OLE::OLE_PPS_TYPE_ROOT);
        }
        return false;
    }
