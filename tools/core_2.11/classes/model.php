...
class model
{
	...
	private function loadDatabase()
	{
		...
		//Подключение файла
		$this->db = include_once($this->config['path']['core'] . '/classes/db.php');
	}
	...
	private function loadTypes()
	{
		...
		$supported_types = include_once($this->config['path']['core'] . '/classes/types/load.php');
	...
	public function loadExtensions()
	{
		...
		foreach ($this->config['extensions'] as $extention_sid => $filename) {
			...
			require($this->config['path']['extensions'] . '/' . $filename);
			//Создаём
