...
////////////////////////////////////////////
	...
	private function getStructureShirtTree_typeTree($root_record_id,$structure_sid,$levels_to_show,$conditions){
		...
		if($root_record_id){
			...
			//Поля для вывода
			$what=array('id','sid','date_public','title','url','left_key','right_key','is_link_to_module','seo_title','seo_keywords','seo_description','seo_changefreq','seo_priority');
			foreach($this->structure[$structure_sid]['fields'] as $field_sid=>$field)
				if($field['group']=='system')
					if(!in_array($field_sid,$what))
						$what[]=$field_sid;
		...
		//Иначе получаем полное дерево структуры
			...
			//Поля для вывода
			$what=array('id','sid','date_public','title','url','left_key','right_key','is_link_to_module','seo_title','seo_keywords','seo_description','seo_changefreq','seo_priority');
			//Также выводим указанные в структуре поля, помеченные как основные
			$what = array_merge($what, $this->getMainFields('rec'));
...
//////////////////////////////////////////////////
	...
	public function getMainFields($structure_sid){
		$fields=array('title', 'url', 'date_public', 'shw');
