...
class field_type_domain extends field_type_default
{
	...
	{
		if( $this->model->config['settings']['domain_switch'] )
			return 'all';
		else
	...
	public function getValueExplode($value, $settings = false, $record = array())
	{
		...
		if ($value == '|' . $this->model->extensions['domains']->domain['id'] . '|')
			return array($this->model->extensions['domains']->domain);
		
		//Все домены
		if($value == 'all'){
			//Варианты значений
			$recs = $this->model->execSql('select `id`,`title`,`host` from `' . $this->model->extensions['domains']->table_name . '` where `active`=1 order by `pos`');
		...
		//Указан домен
			...
			$res  = array();
			//Варианты значений
			$recs = $this->model->execSql('select `id`,`title`,`host` from `' . $this->model->extensions['domains']->table_name . '` where `id` in ("' . implode('", "', $arr) . '") and `active`=1 order by `pos`');
		}
		//Готово
		return $recs;
	}
	...
	public function getAdmValueExplode($value, $settings = false, $record = array())
	{
		...
		$domain_switch = $this->model->config['settings']['domain_switch'];
		
		$res = array();
		
		if ($domain_switch) {
			//Варианты значений
			$variants = $this->model->execSql('select `id`,`title` from `' . $this->model->extensions['domains']->table_name . '` where `active`=1 order by `title`');
			...
			//Варианты значений
			$variants = array(
				$this->model->extensions['domains']->domain
			);
		}
