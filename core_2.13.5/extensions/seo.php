...
*/
			...
			if ($module->structure) {
				foreach ($module->structure as $structure_sid => $structure) {
					foreach ($fields as $field)
						$this->model->modules[$module_sid]->structure[$structure_sid]['fields'][$field['sid']] = $field;
				}
			}
	}
	...
	public function crawlerCount()
	{
		...
		if (!$this->model->config['settings']['count_crawlers'])
			return false;
			...
			if ($browser->crawler) {
				...
				if ($field) {
					$this->model->makeSql(array(
						'tables' => array(
							$this->model->modules[$this->model->ask->module]->getCurrentTable($this->model->ask->structure_sid)
						),
						'fields' => array(
				...
				$f = fopen($this->model->config['path']['www'] . '/crawlers.log', 'a+');
				fwrite($f, date("Y-m-d H:i:s") . '|' . $this->model->extensions['domains']->domain['host'] . '|' . $browser->browser . '|' . $this->model->ask->original_url . '|' . @$sql . "\r\n");
				fclose($f);
				
				chmod($this->model->config['path']['www'] . '/crawlers.log', 0775);
			}
		}
	}
	...
	{
		$tree = $this->model->prepareShirtTree(false, 'rec', false, 10, $conditions = array(
			'and' => array(
				'`shw`=1'
			)
		));
