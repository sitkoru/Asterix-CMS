...
<p>
	...
	function start_core_update(){literal}{{/literal}
		...
		$j('#acms_core_update_status').fadeIn('fast').text('Ожидаю ответа сервера...');
		$j.post('/', $j('#acms_core_update_form').serialize(), function(data){
			$j('#acms_core_update_status').text( data );
		})
