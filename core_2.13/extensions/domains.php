		...
		foreach ($this->model->modules as $module_sid => $module)
			if ($module->structure) {
				foreach ($module->structure as $structure_sid => $structure) {
					$this->model->modules[$module_sid]->structure[$structure_sid]['fields']['domain'] = array(
						'sid' => 'domain',
						'group' => 'system',
						'type' => 'domain',
						'title' => 'Домен'
	...
	private function defineCurrentDomain()
	{
		...
		$this->domain = $this->model->makeSql(array(
			...
			'where' => array(
				'and' => array(
					'
`host`="' . mysql_real_escape_string($host) . '" or
`host2`="' . mysql_real_escape_string($host) . '" or
`host3`="' . mysql_real_escape_string($host) . '" or
`host4`="' . mysql_real_escape_string($host) . '"
',
					'access' => '1'
				)
			)
	...
	{
		$this->model->config['path']['templates'] .= '/' . $this->domain['templates'];
	}
