		...
		{foreach from=$field.value.fields item=rec key=key}
			<tr>
				...
				<!--		<option value="check"{if $rec.type eq 'check'} selected="selected"{/if}>Галочка</option>-->
						<option value="radiolist"{if $rec.type eq 'radiolist'} selected="selected"{/if}>Выбор одного варианта из списка</option>
						<option value="checklist"{if $rec.type eq 'checklist'} selected="selected"{/if}>Выбор нескольких вариантов из списка</option>
					</select>
				</td>
				<td>
					<input type="text" name="{$field.sid}[fields][{$key}][default]" value="{$rec.default}" style="width:150px" />
					<br />* для списков перечисляйте варианты через запятую.
				</td>
