	...
	public function toValue($value_sid, $values, $old_values = array(), $settings = false, $module_sid, $structure_sid){
		...
		foreach( $values[$value_sid]['name'] as $i => $value)
			...
			//Файл передан
				...
				$name = acmsFiles::unique( $values[$value_sid]['name'][$i], $this->model->config['path']['www'] . $this->model->config['path']['public_images'] );
				...
				//Загружаем файл
				$filename = acmsFiles::upload( $values[$value_sid]['tmp_name'][$i], $this->model->config['path']['www'] . $this->model->config['path']['public_images'] . '/' . $name );
