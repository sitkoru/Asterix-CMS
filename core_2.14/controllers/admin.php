...
class controller_admin extends default_controller
{
    ...
    private function preloadForm($action, $record)
    {
        ...
        $action_result['content_template_file'] = $tmpl->correctTemplatePackPath(
            $action_result['content_template_file'],
            'admin_templates'
        );
...
//		pr( $action );
        ...
        if ($action == 'addRecord') {
            ...
            $record = model::$modules[$this->vars['module_sid']]->getRecordById(
                $this->vars['structure_sid'],
                $this->vars['record']
            );
            if ($record) {
    ...
    private function getGroups($action, $record)
    {
        ...
        if ($action == 'settings') {
            ...
            if (!IsSet(model::$modules[model::$ask->module])) {
                log::stop(
                    '404 Not Found',
                    'Произошло обращение к несуществующему модулю',
                    'Вполне возможно, ссылка на модуль была удалёна из дерева сайта вручную. <br />Включите режим Разработки для автоматического исправления ошибки.'
                );
            }
            $fields = model::$modules[model::$ask->module]->prepareInterface(
                $action,
                array('id' => $record['id']),
                false
            );
    ...
    private function updateSettings()
    {
        ...
        foreach ($sets as $set) {
            if (IsSet($this->vars[$set['var']]) or ($set['type'] == 'check')) {
                ...
                $sql = 'update `settings` set ' . $in_str . ' where ' . model::pointDomain(
                    ) . ' and `var`="' . mysql_real_escape_string($set['var']) . '"';
