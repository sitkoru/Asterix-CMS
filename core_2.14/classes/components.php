		...
		if( $params['chop_to_pages'] ) {
			...
			if( $num_of_pages>1 ) {
				...
				if( count( model::$ask->mode )>0 ) {
					$modifiers = strip_tags( '.' . implode( '.', model::$ask->mode ) );
				}
				...
				//Предыдущая страница
				$pages['prev']['url'] = model::$ask->rec['url'] . $modifiers . '.' . $prev . '.' . model::$ask->output_format . $get_vars_string;
				$pages['prev']['num'] = $prev;

				// Следующая страница
				$pages['next']['url'] = model::$ask->rec['url'] . $modifiers . '.' . $next . '.' . model::$ask->output_format . $get_vars_string;
				$pages['next']['num'] = $next;

				// Префикс для указания количества записей на странице
				if( $get_vars['items_per_page'] ) UnSet($get_vars['items_per_page']);
				$pages['items_per_page_url'] = model::$ask->rec['url'] . $modifiers . '.' . intval( $current_page ) . '.' . model::$ask->output_format . ($get_vars ? '?' . implode( '&', $get_vars ) . '&items_per_page=' : '?items_per_page=');

				//Другие страницы
				for( $i = 0; $i<$num_of_pages; $i++ ) {
					$pages['items'][$i]['url'] = model::$ask->rec['url'] . $modifiers . '.' . $i . '.' . model::$ask->output_format . $get_vars_string;

					// Скрываем большие списки страниц
					if( (abs( $i-$current_page )>5) && ($i>0) && ($i+2<$num_of_pages) )
