...
class model
{
    ...
    public static function prepareMainMenu($levels_to_show)
    {
        ...
        //Получаем дерево
        $recs = self::prepareShirtTree('start', 'rec', false, $levels_to_show, [
            'and' => [
                '`show_in_menu`=1',
                '`shw`=1'
            ]
...
//		pr_r($recs);
    ...
    public static function prepareShirtTree(
        $module_sid = 'start',
        $structure_sid = 'rec',
        $root_record_id = false,
        $levels_to_show = 2,
        $conditions = ['and' => ['`shw`=1']]
    ) {
        if (is_object(self::$modules[$module_sid])) {
            return self::$modules[$module_sid]->getModuleShirtTree($root_record_id, $structure_sid, $levels_to_show,
                $conditions);
        }
    }
            ...
            if ($structures) {
                ...
                foreach ($structures as $structure_sid => $structure) {
                    if (!$record) {
                        ...
                        $record = self::makeSql(
                            [
                                'tables' => [model::$modules[$prefered_module]->getCurrentTable($last_structure_sid)],
                                'where'  => ['and' => ['url' => $where]]
                            ],
                            'getrow'
                        );
    ...
    function check_no_www()
    {
        if (model::$settings['no_www']) {
            ...
            if (substr_count($_SERVER['HTTP_HOST'], 'www.') and (model::$settings['no_www'] == 'no_www')) {
                ...
                //Без www
            } elseif (!substr_count($_SERVER['HTTP_HOST'],
                    'www.') and (model::$settings['no_www'] == 'www') and (substr_count($_SERVER['HTTP_HOST'], '.') < 2)
            ) {
