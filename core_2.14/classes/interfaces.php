...
use app\classes\ElasticHelper;

trait interfaces
{

    public function getInterface_load($module)
    {
    ...
    public function getInterface($prepare, $params, $public = false)
    {
        ...
        if (IsSet($this->interfaces[$prepare])) {
            ...
            $fields = $this->getInterface_fields($record, $prepare, $public);
    ...
    public function controlInterface($interface, $params, $public = false)
    {
        ...
        if ($this->interfaces[$interface]['auth']) {
            $access = $this->checkAccessToManageRecord(model::$ask->rec, $this->interfaces[$interface]);
            if (!$access)
        ...
        $fields = $this->getInterface_fields($params, $interface, $public);
    ...
    public function getInterface_fields($record = false, $interface = false, $public = false)
    {
        $structure_sid = $this->interfaces[$interface]['structure_sid'];
        if (!$structure_sid) $structure_sid = model::$ask->structure_sid;
...
//				$this->interfaces[$interface]['fields']=$fields;
    ...
    public function editRecord($values, $structure_sid = 'rec', $conditions = false)
    {
     ...
     /*   ini_set('display_errors', 1);
        ...
        //Обновляем поддерево
        $this->updateChildren($structure_sid, $data_before, $data_after, $url);
    ...
    public function checkAccessToManageRecord($record, $interface)
    {
