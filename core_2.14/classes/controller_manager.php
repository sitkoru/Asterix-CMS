...
class controller_manager
{
    ...
    private function defineController()
    {
        ...
        foreach ($this->controllers as $sid => $controller) //Если допустимый метод
        {
    ...
    private function feedback()
    {
        ...
        $footer = 'Сообщение отослано со страницы: <a href="' . $_SERVER['HTTP_REFERER'] . '">' . urldecode(
        ...
        foreach (model::$modules as $module_sid => $module) {
            if (IsSet($module->structure)) {
                if (!$module->structure['rec']['hide_in_tree'] and !$module->structure['rec']['hide_in_sitemap']) {
                    ...
                    $current = model::execSql(
                        'select ' . $fields . ' from `' . $module->getCurrentTable('rec') . '` where `shw`=1',
                        'getall'
                    );
