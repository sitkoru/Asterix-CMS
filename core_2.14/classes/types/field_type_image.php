    ...
    public function toValue(
        ...
        */
        ...
        if ($values[$value_sid . '_delete']) {
            ...
            $dir_path = model::$config['path']['public_images'] . '/' . $module_prototype . '/' . $structure_sid . str_pad(
                    $values['id'],
                    6,
                    '0',
                    STR_PAD_LEFT
            ...
            if (!$created) {
                log::stop(
                    ...
                    'Нет доступа для создания папки',
                    model::$config['path']['www'] . $dir_path
                );
            }
            ...
            //Загружаем файл
            $filename = acmsFiles::upload(
                $values[$value_sid]['tmp_name'],
                model::$config['path']['www'] . $dir_path . '/' . $name,
                '0775',
            ...
            $data = $acmsImages->resize(
                $filename,
                false,
                $settings['resize_type'],
            ...
            if (IsSet($values[$value_sid . '_watermark'])) {
                if ($values[$value_sid . '_watermark']['size'] > 0) {
                    $acmsImages->put_watermark(
                        $filename,
                        false,
                        $values[$value_sid . '_watermark'],
                        $values[$value_sid . '_watermark_side']
                    );
                }
            }
                        ...
                        if (class_exists('acmsWatermark')) {
                            ...
                            if (($pre_image_width >= $min_size) || ($pre_image_height >= $min_size)) {
                                if (acmsWatermark::isWatermarkNeeded_image(
                                    ) || acmsWatermark::isWatermarkNeeded_interface()
                                ) {
...
//			$rec['pre'] = $settings['pre'];
    ...
    {
        if ($module_sid) {
            $sql = 'select DATA_TYPE from information_schema.COLUMNS where TABLE_SCHEMA="' . model::$config['db']['system']['name'] . '" and TABLE_NAME="' . model::$modules[$module_sid]->getCurrentTable(
                    $structure_sid
                ) . '" and COLUMN_NAME="' . $field_sid . '"';
            $res = model::execSql($sql, 'getrow');
            if ($res['DATA_TYPE'] != 'text') {
                $sql = 'alter table `' . model::$modules[$module_sid]->getCurrentTable(
                        $structure_sid
                    ) . '` modify ' . $this->creatingString($field_sid);
    ...
    private function old2new($value, $settings)
    {
        ...
        //Данные
        if (!is_array($value)) {
            list($rec['path'], $rec['type'], $rec['size'], $rec['width'], $rec['height'], $rec['title'], $rec['realname'], $rec['colors']) = explode(
                '|',
                $value
            );
