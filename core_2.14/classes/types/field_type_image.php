...
class field_type_image extends field_type_default
{
	...
	public function toValue($value_sid, $values, $old_values = array(), $settings = false, $module_sid, $structure_sid){
		...
		//Файл передан
			...
			$name = acmsFiles::unique( $values[$value_sid]['name'], model::$config['path']['images'] );
			...
			//Загружаем файл
			$filename = acmsFiles::upload( $values[$value_sid]['tmp_name'], model::$config['path']['images'] . '/' . $name );
			...
			$data['size'] = filesize( model::$config['path']['images'] . '/'. $name );
