...
{foreach from=$action.recs item=rec key=key}
	<li>
		{$rec.title} [{$rec.id}] 
		<a href="#" OnClick="$('#{$module.id}_structure').toggle('fast'); return false;">структура</a>
		<a href="#" OnClick="$('#{$module.id}_data').toggle('fast'); return false;">данные</a>
...
{foreach from=$action.recs_all item=rec key=key}
	<li>
		{$rec.title} [{$rec.id}] - <a href="#" OnClick="$('#add_module_{$rec.id}').toggle('fast'); return false;">подключить</a>
