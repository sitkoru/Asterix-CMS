...
{if $field.value.path}
			<span style="display:block; color:grey; font-size:0.8em;">
				...
				<input type="checkbox" name="{$field.sid}_delete" id="{$field.sid}_delete" value="1" style="margin-top:10px;"{if $field.required} required="required"{/if} />
				<label class="control-label" for="{$field.sid}_delete" style="display:inline;">удалить файл</label>
			</span>
{/if}
		</div>
	</div>
